...
// Replace Node.js crypto with browser-compatible functions
function encrypt(text) {
    // Simple XOR encryption with base64 encoding
    let output = '';
    const key = 'your-encryption-password'; // Use the same password
        ...
        if (toggle.contains(event.target)) {
            ...
            if (selectedOption.tagName === 'LI') {
                ...
                if (dropdown.classList.contains('dropdown-memory')) {
                    ...
                    localStorage.setItem(`dropdown-selected-text-${dropdownId}`, newText);
                    location.reload();
                }
            }
        } else {
