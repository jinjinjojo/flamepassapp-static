/**
 * Completely Revised Game Page Handler
 * Fixes issues with display, buttons, search, and organization
 */
...
async function initGamePage(slug) {
  ...
  try {
    ...
    // Remove any purple category text links that might exist
    removeUnwantedCategorySections();
...
function updateGameDetails(game, allGames) {
  ...
  // Create enhanced game UI with proper parallax background
  const gameImageUrl = game.img || '/assets/game-placeholder.jpg';
  
  // Build the game info HTML
  let gameInfoHTML = `
  ...
  // Handle cloud games differently with service providers
  if (game.category === 'cloud') {
    // Service providers section for cloud games
    gameInfoHTML += `
      <div class="service-providers-section">
        <h3 class="providers-title"><i class="fa-solid fa-cloud"></i> Play on:</h3>
        <div class="service-providers-grid">`;
    
    // Add Boosteroid if available
    if (game.serviceProviders && game.serviceProviders.boosteroid) {
      gameInfoHTML += `
        <div class="provider-card">
          <div class="provider-logo">
            <img src="/assets/providers/boosteroid.png" alt="Boosteroid">
          </div>
          <h4 class="provider-name">Boosteroid</h4>
          <div class="provider-buttons">
            <a href="${game.serviceProviders.boosteroid}" class="provider-play-button" target="_blank">
              <i class="fa-solid fa-play"></i> Play Now
            </a>
            <a href="#" class="provider-proxy-button" data-provider="boosteroid" data-url="${game.serviceProviders.boosteroid}">
              <i class="fa-solid fa-lock"></i> Use Proxy
            </a>
          </div>
        </div>`;
    }
    
    // Add GeForce Now if available
    if (game.serviceProviders && game.serviceProviders.geforcenow) {
      gameInfoHTML += `
        <div class="provider-card">
          <div class="provider-logo">
            <img src="/assets/providers/geforcenow.png" alt="GeForce Now">
          </div>
          <h4 class="provider-name">GeForce Now</h4>
          <div class="provider-buttons">
            <a href="${game.serviceProviders.geforcenow}" class="provider-play-button" target="_blank">
              <i class="fa-solid fa-play"></i> Play Now
            </a>
            <a href="#" class="provider-proxy-button" data-provider="geforcenow" data-url="${game.serviceProviders.geforcenow}">
              <i class="fa-solid fa-lock"></i> Use Proxy
            </a>
          </div>
        </div>`;
    ...
    // Add Xbox Cloud if available
    if (game.serviceProviders && game.serviceProviders.xboxcloud) {
      gameInfoHTML += `
        <div class="provider-card">
          <div class="provider-logo">
            <img src="/assets/providers/xboxcloud.png" alt="Xbox Cloud Gaming">
          </div>
          <h4 class="provider-name">Xbox Cloud</h4>
          <div class="provider-buttons">
            <a href="${game.serviceProviders.xboxcloud}" class="provider-play-button" target="_blank">
              <i class="fa-solid fa-play"></i> Play Now
            </a>
            <a href="#" class="provider-proxy-button" data-provider="xboxcloud" data-url="${game.serviceProviders.xboxcloud}">
              <i class="fa-solid fa-lock"></i> Use Proxy
            </a>
          </div>
        </div>`;
    ...
    // Close service providers divs
    gameInfoHTML += `
        </div>
      </div>`;
  } else {
    // Regular game buttons for non-cloud games
    gameInfoHTML += `
  ...
  // Update the game info HTML content
  ...
  updateGameButtons(gameInfo, game);
...
function initParallaxEffect() {
  ...
  // Initial positioning
  parallaxBg.style.transform = 'scale(1.1)';
  
  // Set up scroll listener
  window.addEventListener('scroll', () => {
    const scrollPosition = window.scrollY;
    // Use scale(1.1) to maintain the initial size while translating
    parallaxBg.style.transform = `translateY(${scrollPosition * 0.3}px) scale(1.1)`;
  });
}

// Update buttons with game URL and login status
function updateGameButtons(gameInfo, game) {
  // Check if user is logged in
  const isLoggedIn = typeof window.isLoggedIn === 'function' ? window.isLoggedIn() : checkLoginStatus();
  ...
  if (game.category !== 'cloud') {
    ...
    if (proxyButton && game.url) {
      ...
      // Update the button based on login status
      if (isLoggedIn) {
        // User is logged in, remove lock icon
        proxyButton.innerHTML = `
          <i class="fa-solid fa-shield"></i>
          Play in Flamepass Proxy
        `;
      } else {
        // User is not logged in, show lock icon
        proxyButton.innerHTML = `
          <i class="fa-solid fa-lock lock-icon"></i>
          Play in Flamepass Proxy
        `;
    ...
    // Handle cloud provider proxy buttons
    const proxyButtons = document.querySelectorAll('.provider-proxy-button');
    proxyButtons.forEach(button => {
      ...
      if (isLoggedIn) {
        button.innerHTML = `<i class="fa-solid fa-shield"></i> Use Proxy`;
      } else {
        button.innerHTML = `<i class="fa-solid fa-lock"></i> Use Proxy`;
      }
      
      // Remove any existing click handlers
      const newButton = button.cloneNode(true);
      button.parentNode.replaceChild(newButton, button);
      newButton.addEventListener('click', handleProviderProxyGame);
    });
  }
}
...
// Handle cloud provider proxy clicks
function handleProviderProxyGame(e) {
  e.preventDefault();

  // Get provider URL from data attribute
  const providerUrl = e.currentTarget.getAttribute('data-url');
  const provider = e.currentTarget.getAttribute('data-provider');
  
  if (!providerUrl) {
    console.error(`Game URL not found for ${provider} proxy`);
    return;
  }
  ...
  if (!isLoggedIn) {
    ...
    // User is logged in, launch the provider
    launchProviderInProxy(providerUrl, provider);
  }
}
...
// Launch cloud gaming provider in proxy
function launchProviderInProxy(providerUrl, provider) {
  if (!providerUrl) return;

  // Method 1: Use the executeSearch function if available
  if (typeof window.executeSearch === 'function') {
    // Call executeSearch with proxy=true
    window.executeSearch(providerUrl, true);
    return;
  }

  // Method 2: Redirect to the &.html page with appropriate parameters
  const encodedUrl = encodeURIComponent(providerUrl);
  window.location.href = `/&.html?q=${encodedUrl}&proxy=true&provider=${provider}`;
}

// Function to show login modal
function showLoginModal() {
  // Use site's auth controller if available
  ...
  if (authModal) {
    ...
    console.warn('No login modal found');
  }
}
...
function updateGameGrid(container, games) {
  ...
  games.forEach((game) => {
    ...
    // Create card HTML with proper styling
    gameCard.innerHTML = `
...
function initAuthModal() {
  ...
  if (!authModal || !closeAuthModal || !authLoginBtn) return;
  ...
  // Login button functionality - delegate to auth controller if available
  authLoginBtn.addEventListener('click', () => {
    // Close the modal
    authModal.classList.remove('active');
    
    // Use the site's auth controller if available
    if (typeof window.showLoginPopup === 'function') {
      window.showLoginPopup();
    } else {
...
// Remove any unwanted category sections with purple text
function removeUnwantedCategorySections() {
  // Find elements with "Cloud Games" or similar text that aren't in our container
  const unwantedSections = document.querySelectorAll('a[href*="cloud"]');
  unwantedSections.forEach(section => {
    // Only remove if it's not inside our managed game container
    if (!section.closest('.game-container') && !section.classList.contains('view-all-link')) {
      // Check if it's a category header by color
      const style = window.getComputedStyle(section);
      if (style.color.includes('rgb(128, 0, 128)') || style.color.includes('purple')) {
        // Remove it and potentially its parent container
  ...
  // Find other potential category sections
  const potentialCategoryLinks = document.querySelectorAll('a[href*="browser"], a[href*="html5"], a[href*="emulator"]');
  potentialCategoryLinks.forEach(link => {
    if (!link.closest('.game-container') && !link.classList.contains('view-all-link')) {
      // Check if it looks like a category header
      const style = window.getComputedStyle(link);
      const text = link.textContent.trim().toLowerCase();
      
      if (text.includes('games') && (style.color.includes('purple') || style.color.includes('rgb(128, 0, 128)'))) {
        // Remove it and potentially its parent container
