...
function updateGameDetails(game, allGames) {
  ...
  // Update metadata items (Category and Genre)
  const metadataItems = document.querySelectorAll('.metadata-item');
  if (metadataItems.length > 0) {
    // Update category (first metadata item)
    const categoryItem = metadataItems[0];
    const categoryValue = categoryItem.querySelector('.metadata-value');
    if (categoryValue && game.category) {
      categoryValue.textContent = game.category.charAt(0).toUpperCase() + game.category.slice(1) + ' Game';
    }

    // Update genre if available (second metadata item)
    if (metadataItems.length > 1) {
      const genreItem = metadataItems[1];
      const genreValue = genreItem.querySelector('.metadata-value');
      if (genreValue && game.genre) {
  ...
  // Update game description - don't duplicate if a custom description exists
  const gameDesc = document.querySelector('.game-description');
  if (gameDesc) {
    // Remove any existing custom description
    const existingCustomDesc = gameDesc.querySelector('.custom-description');
    if (existingCustomDesc) {
      existingCustomDesc.remove();
    }
