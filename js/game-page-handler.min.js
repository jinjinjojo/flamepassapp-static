 ...
 * Updates existing DOM elements instead of creating new ones
 ...
 * - Proxy integration
 * - Authentication handling (using existing auth-controller)
 */
...
async function initGamePage(slug) {
  ...
  try {
    ...
    updateRelatedContent(currentGame, allGames);
...
/**
 * GAME DETAILS UPDATING AND UNBLOCKING TIPS
 */

// Update game details instead of recreating them
function updateGameDetails(game, allGames) {
  ...
  // Update category
  const categoryValue = document.querySelector('.metadata-value');
  if (categoryValue && game.category) {
    categoryValue.textContent = game.category.charAt(0).toUpperCase() + game.category.slice(1) + ' Game';
  }

  // Update genre if available
  const genreItem = document.querySelectorAll('.metadata-item')[1];
  if (genreItem && game.genre) {
    const genreValue = genreItem.querySelector('.metadata-value');
    if (genreValue) {
  ...
  // Update game image
  const gameImage = document.querySelector('.game-detail-image');
  if (gameImage) {
    gameImage.src = game.img || '/assets/game-placeholder.jpg';
    gameImage.alt = game.name;
  }

  // Update custom game description if available
  const gameDesc = document.querySelector('.game-description');
  if (gameDesc && game.description) {
    // Check if custom description already exists
    let customDesc = gameDesc.querySelector('.custom-description');

    if (!customDesc) {
      // Create custom description if it doesn't exist
      customDesc = document.createElement('div');
      customDesc.className = 'custom-description';
      gameDesc.appendChild(customDesc);
    }
...
function updateRelatedContent(currentGame, allGames) {
  ...
  // Limit to 4 games and update grid
  relatedGames = relatedGames.slice(0, 4);
  updateGameGrid(relatedContainer, relatedGames);
}
...
function updateTrendingGames(allGames) {
  ...
  // Update trending games
  updateGameGrid(trendingContainer, trendingGames);
}
...
function updateGameGrid(container, games) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    // If we have a game but no card, skip (we don't want to create new elements)
    if (!card) continue;
    ...
    if (game) {
      ...
      if (img) {
        img.src = game.img || '/assets/game-placeholder.jpg';
        img.alt = game.name;
      }

      // Update game name
      const gameName = card.querySelector('.game-card-overlay h3');
      if (gameName) {
      ...
      // No game data for this card, hide it
...
async function performSearch(query) {
  ...
  try {
    ...
    if (filteredGames.length === 0) {
      ...
      // Get template for search results if it exists
      const searchResultTemplate = document.querySelector('.search-result-item');
      const hasTemplate = searchResultTemplate !== null;

      // Method to create or update result items
      const createOrUpdateResultItem = (game, index) => {
        let resultItem;

        if (hasTemplate && index === 0) {
          // Use the existing template for the first item
          resultItem = searchResultTemplate;
        } else {
          // For remaining items, clone the template if it exists
          if (hasTemplate) {
            resultItem = searchResultTemplate.cloneNode(true);
          } else {
            // Create new item if no template exists
        ...
        const categoryPill = resultItem.querySelector('.category-pill');
        if (categoryPill) {
        ...
        const publisher = resultItem.querySelector('.publisher');
        if (publisher) {
          if (game.publisher) {
        ...
        // Add to results if not the template or first item
        if (index > 0 || !hasTemplate) {
          searchResults.appendChild(resultItem);
        }
      };

      // Display up to 8 results
      const resultsToShow = filteredGames.slice(0, 8);
      resultsToShow.forEach(createOrUpdateResultItem);
      ...
      if (filteredGames.length > 8) {
        ...
        if (viewAllLink) {
          ...
          viewAllLink.href = `/g.html?search=${encodeURIComponent(query)}`;
          viewAllLink.innerHTML = `View all ${filteredGames.length} results <span class="material-symbols-outlined">arrow_forward</span>`;
          viewAllLink.style.display = '';
        } else {
          // Create view all link if it doesn't exist
        ...
        // Hide view all link if it exists
        const viewAllLink = document.querySelector('.view-all-results');
        if (viewAllLink) {
  ...
  if (loadingContainer && gameContainer) {
    ...
    fetchGames().then(games => {
      ...
      if (game) {
        ...
        updateRelatedContent(game, games);
