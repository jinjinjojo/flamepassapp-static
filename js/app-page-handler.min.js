/**
 * App Page Handler (Fixed V3)
 * Specifically handles the apps page content and authentication,
 * while still loading apps in the background
 */

// Wait for DOM to be ready
document.addEventListener('DOMContentLoaded', function() {
  // Check if this is the apps page
  if (window.location.pathname !== '/a.html') return;
  
  console.log("App page handler initialized");
  
  // Check if user is logged in
  const loggedIn = typeof isLoggedIn === 'function' ? isLoggedIn() : false;
  
  if (!loggedIn) {
    // Protect the page but still load apps in background
    protectAppsPage();
  }
});

// Protect apps page functionality for non-logged-in users
function protectAppsPage() {
  console.log("Protecting apps page");
  
  // 1. DO NOT intercept fetch requests - allow apps to load in background
  ...
  if (appsContainer) {
    ...
    blackOverlay.className = 'apps-black-overlay';
    ...
    lockText.textContent = 'Please login to access Flamepass Apps.';
    ...
    // 3. Disable app search input
    const appsSearchInput = document.querySelector('.appsSearchInput');
    if (appsSearchInput) {
      appsSearchInput.placeholder = "Log in to search for apps...";
      appsSearchInput.disabled = true;
      appsSearchInput.addEventListener('click', function(e) {
        e.preventDefault();
        if (typeof showLoginPopup === 'function') {
          showLoginPopup();
        }
    ...
    // 4. Make sure the overlay handles all clicks
    ...
    // 5. Make sure the sidebar is still usable
